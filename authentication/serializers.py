...
from authentication.models import (
    User, Client, UserProfile, ClientReview, ReplyReview, PasswordResetToken, BlackList)
...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    role = serializers.ChoiceField(
        choices=[('CA', 'Client Admin'), ('CL', 'Client')])
    ...
    password = serializers.CharField(
        ...
        write_only=True,
        error_messages={
            "min_length": "The Password should be at least {min_length} characters",
            "max_length": "The Password should be less than {max_length} characters"
        }
    )
    ...
    def validate(self, data):
        ...
        if not self.do_passwords_match(data["password"], confirmed_password):
            raise serializers.ValidationError({
                "passwords": ("Oops! the passwords do not match")
            })
...
class GoogleAuthSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        return {
            ...
            'user_exists': False,
            'message': 'Welcome to Acres. ' + first_name +
                       '. Ensure to edit your profile for a better experience.'
        }
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not user.is_verified:
            raise serializers.ValidationError({
                "user": "Your email is not verified, please vist your email to proceed with us"
            })
        user = {
            "email": user.email,
            "token": user.token
...
class ChangePasswordSerializer(serializers.ModelSerializer):
    ...
    password = serializers.CharField(
        ...
        error_messages={
            ...
            "max_length": "Password should be less than {max_length} characters"
        }
    )
    ...
    def validate(self, data):
        ...
        try:
            ...
            if not user_token.is_valid:
                raise serializers.ValidationError({
                    "token": "This token is no longer valid, please request for a \
                    new one"
                })

        except PasswordResetToken.DoesNotExist:
            raise serializers.ValidationError({
                "token": "We couldn't find that token in our database"
            })
